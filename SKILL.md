...
4. Download the logo file to `assets/logo-original.[ext]` following the **tiered logo capture rule** in `references/image-handling.md` under "LOGO CAPTURE — TIERED WITH SOFT-WARNING FALLBACK" (added v0.7.2). Tier 1: `<img>` inside header/hero/nav with logo-matching alt/filename/class. Tier 2: OG image, apple-touch-icon, or favicon ≥128×128. Tier 3: type-based wordmark in captured brand font and primary color. Tier 3 triggers a Phase 6 soft warning but NEVER fails the build. If a raster logo is obtained, also save a PNG copy at `assets/logo.png` for node-vibrant analysis.
...
**OG image color rule (v0.7.2).** The OG image background color MUST pull from `profile-draft.brandPalette.primary` — the same captured hex that drives `--color-primary` in `style.css`. Hardcoded default colors are forbidden. If `brandPalette.primary` is missing or null, fall back to neutral dark gray `#1a1a1a` — never blue, never an accent color, never a "GRM brand color". Phase 6 verifies the OG image primary color matches the captured brand primary within ±20° hue tolerance. Full spec in `references/seo-geo.md` under "OG IMAGE COLOR — MUST PULL FROM CAPTURED BRAND PALETTE".
